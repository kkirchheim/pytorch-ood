...
class VOSRegLoss(nn.Module):
    ...
    For initialisation of :math:`\\phi` and :math:`weights_energy`  see `GitHub <https://github.com/deeplearning-wisc/vos/blob/a449b03c7d6e120087007f506d949569c845b2ec/classification/CIFAR/train_virtual.py#L132>`__ .
    ...
    def _regularization(self, logits, y):
        ...
        if 'cuda' in self.device:
            labels_for_lr = torch.cat((torch.ones(len(energy_x_in)).cuda(),
                                    torch.zeros(len(energy_v_out)).cuda()), -1)
        else:
    ...
    def _energy(self, logits,dim=1,keepdim=False):
        ...
        return -(m + torch.log(torch.sum(
            F.relu(self.weights_energy.weight) * torch.exp(value0), dim=dim, keepdim=keepdim)))

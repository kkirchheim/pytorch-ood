...
Implementations are often tied to individual papers, method interfaces vary considerably, and evaluation protocols are difficult to reproduce across projects. `pytorch-ood` addresses this problem by providing a unified, research-oriented software library for OOD detection in the PyTorch ecosystem.
...
The central architectural principle of `pytorch-ood` is interface unification. Conceptually, the package separates the Neural Netowkr from the detector applied to that network and from the evaluation logic used to assess detection quality.
This decomposition is simple, but it has practical consequences.
It allows a single trained model to be paired with multiple detectors, makes experimental comparisons easier to script, and reduces the amount of method-specific glue code.
It also provides a natural extension point for future work: new detectors can be integrated into the existing ecosystem without redesigning dataset or evaluation code.

This emphasis on unification is paired with breadth. The package does not merely offer one common interface for a small set of algorithms; rather, it attempts to cover a sufficiently broad set of methods and datasets that the interface becomes genuinely useful in day-to-day research. That combination of common abstraction and broad coverage is, in our view, the main software contribution of the project.
...
`pytorch-ood` occupies a somewhat different point in the design space. Its primary goal is not to prescribe a single evaluation regime, or a particular application domain, but to provide a flexible PyTorch-native library in which detectors, losses, models, datasets, and benchmarks share a coherent interface.
...
The package is therefore best understood not as a replacement for benchmark-centric frameworks, but as complementary infrastructure that prioritizes modularity and reuse.
